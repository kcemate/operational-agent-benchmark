...
26. **Spend authorization has no host-backed conversational verifier.** OAB cannot prove that a quoted chat message reference exists, is immutable, or approves these exact controls, so `approval-request --conversation-approval-reference` is refused with `conversation_approval_not_host_verified` and an unverified conversational receipt cannot authorize `resume`. Conversational no-spend preview and explicit user approval are still required before anything runs; the only spend-capable authorization is the externally signed Ed25519 stage approval, whose assurance is exactly that of the approver's key handling — OAB does not attest who holds it.
